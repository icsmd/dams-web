                                ...
                                <v-col>
                                    ...
                                    >
                                        <template v-slot:item="{ props, item }">
                                            <v-list-item v-bind="props" :title="`${item.raw.agency_abbr} | ${item.raw.agency_name}`"></v-list-item>
                                        </template>
                                        <template v-slot:label>
                                            <div>
...
const getData = async () => {
    try {
        const response = await axios.get(`v1/person/${params.actionParams.person_id}`);
            ...
            model.type = fectchData.value[0].type;
            model.rfid = fectchData.value[0].rfid;
            model.agency = fectchData.value[0].agency_details.id;
    } catch (error) {
        console.error("Error fetching list:", error);
    }
};
