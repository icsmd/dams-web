...
const actions = {
	async paginate(
		...
		const { data, meta } = await axios.get('v1/person_desig')
	...
	async update({ commit }, model) {
		const response = await axios.patch(`v1/person_desig/${model.pd_id}`, model)

		return response
	},
	async save({ commit }, model) {
		const response = await axios.post('v1/person_desig', model)

		return response
	},
	async destroy({ commit }, model) {
		const response = await axios.delete(`v1/person_desig/${model.pd_id}`)
