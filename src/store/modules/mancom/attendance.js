...
const actions = {
	...
	async update({ commit }, model) {
		const response = await axios.patch(`v1/attendance/${model.attendance_id}`, model)

		return response
	},
	async save({ commit }, model) {
		const response = await axios.post('v1/attendance', model)

		return response
	},
	async destroy({ commit }, model) {
		const response = await axios.delete(`v1/attendance/${model.attendance_id}`)
